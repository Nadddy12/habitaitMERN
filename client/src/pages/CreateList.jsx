  ...
  const handleImageSubmit = (e) => {
    if(files.length > 0 && files.length + formData.imageUrls.length < 7) {
      ...
      Promise.all(promises).then((urls) => {
        ...
        setImageUploadError('Échec du téléchargement des images (2 Mo maximum par image)')
        setUploading(false);
      });
    } else {
    ...
    <main className='p-3 max-w-4xl mx-auto'>
      ...
      <form className='flex flex-col sm:flex-row gap-4'>
        ...
        <div className='flex flex-col gap-4 flex-1'>
          ...
          <p className='text-red-700 text-sm p-3 text-center'>
          ...
          {formData.imageUrls.length > 0 && formData.imageUrls.map((url , index) => (
            <div key={url} className='flex justify-between p-3 border items-center'>
              ...
              <button 
                ...
                className='p-3 text-red-700 rounded-lg uppercase hover:opacity-70 border border-red-700'>
                Supprimer
              </button>
            </div>
            ))}
          <button className='p-3 bg-slate-700 text-white rounded-lg uppercase hover:opacity-95 disabled:opacity-80'>Créer l'annonce</button>
        </div>
      </form>
    </main>
  );
